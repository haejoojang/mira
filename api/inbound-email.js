...
module.exports = async (req, res) => {
  ...
  try {
    // Resend's inbound payload shape: adjust field names to match your Resend
    // inbound webhook config (see README — "Configure Resend inbound parsing").
    const { from, text } = req.body;
    const fromEmail = (from || '').match(/[\w.+-]+@[\w-]+\.[\w.-]+/)?.[0]?.toLowerCase();
    if (!fromEmail || !text) return res.status(400).json({ error: 'missing from/text' });
    ...
    if (!profile) return res.status(404).json({ error: 'no matching mira account for this address' });

    const todayISO = new Date().toISOString().slice(0, 10);
    const parsed = await extractWithClaude(text, todayISO);
    const entries = Array.isArray(parsed.entries) ? parsed.entries : [];

    if (entries.length > 0) {
      await supabase.from('cycle_entries').insert(
        entries.map(e => ({ user_id: profile.id, entry_date: e.date, category: e.category || 'other' }))
      );
    }

    await sendConfirmation(profile.email, entries, !!parsed.concern, parsed.concernNote);

    // `text` goes out of scope here and is never written anywhere — this is the
    // entire enforcement mechanism for "we don't store what you write."
    return res.status(200).json({ ok: true, savedCount: entries.length });
  } catch (err) {
    console.error('inbound-email error:', err.message); // never log `text` itself
